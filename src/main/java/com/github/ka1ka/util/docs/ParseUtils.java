...
import com.github.ka1ka.util.docs.exception.JavaFileNotFoundException;
import com.github.ka1ka.util.docs.parser.*;
...
public class ParseUtils {
     ...
     */
    public static void parseClassNodeByType(File inJavaFile, ClassNode rootClassNode, Type classType, String methodName) {
        ...
        if (TYPE_MODEL.equals(unifyClassType)) {
            if (classType instanceof ClassOrInterfaceType) {
                ...
                try {
                    ...
                    parseClassNode(modelJavaFile, rootClassNode, methodName);
                } catch (JavaFileNotFoundException ex) {
     ...
     * @param classNode
     */
    public static void parseClassNode(File modelJavaFile, ClassNode classNode, String methodName) {
        innerParseClassNode(modelJavaFile, classNode, methodName);
    }

    private static void innerParseClassNode(File modelJavaFile, ClassNode classNode, String methodName) {
        ...
        ParseUtils.compilationUnit(modelJavaFile).
            ...
            if (!extendClassTypeList.isEmpty()) {
                ...
                ParseUtils.parseClassNodeByType(modelJavaFile, extendClassNode, extendClassType, null);
                // 把解析结果复制到子类
            ...
            cl.findAll(FieldDeclaration.class)
                    ...
                    .forEach(fd -> {
                        ...
                        //判断是否为空
                        final boolean notNull = isFieldNotNull(fd, methodName);
                        ...
                        fd.getVariables().forEach(field -> {
                            ...
                            parseFieldNode(fieldNode, modelJavaFile, fieldType, methodName);
                        });
                    });
        });
    ...
    private static boolean isFieldNotNull(FieldDeclaration fd, String methodName) {
        for (AnnotationExpr annotationExpr : fd.getAnnotations()) {
            if (isNotNullAnnotation(annotationExpr.getNameAsString())) {
                return true;
            }
            //增加判断是否存在方法中
            if (isRequired(annotationExpr.getNameAsString())) {
                //如果没有参数  直接返回true
                if (ObjectUtils.isEmpty(methodName)) {
                    return true;
                } else {
                    //todo 参数不为空的情况 判断方法是否在里面
                    if (annotationExpr.getChildNodes().size() > 0) {
                        List<Node> childNodes = annotationExpr.getChildNodes();
                        for (Node childNode : childNodes) {
                            if (childNode.toString().contains("methodName") && childNode.toString().contains(methodName)) {
                                return true;
                            }
                        }
                    }
    ...
    public static boolean isRequired(String required) {
        return required.endsWith("Required");
    }

    private static void parseFieldNode(FieldNode fieldNode, File inJavaFile, Type fieldType, String methodName) {
        ...
        if (TYPE_MODEL.equals(unifyType)) {
            ...
            try {
                ...
                if (!inClassDependencyTree(fieldNode, fieldNode.getClassNode())) {
                    parseClassNode(childNodeJavaFile, childNode, methodName);
                } else {
