...
import com.github.ka1ka.util.docs.*;
import com.github.ka1ka.util.docs.consts.ChangeFlag;
...
public abstract class AbsControllerParser {
    ...
    private void parseMethodDocs(ClassOrInterfaceDeclaration c) {
        c.findAll(MethodDeclaration.class).stream()
                ...
                .forEach(m -> {
                    ...
                    /*//todo 过滤 增加对象判断 获取对象的参数
                    m.getParameters().forEach(p -> {
                        String paraName = p.getName().asString();
                        ParamNode paramNode = requestNode.getParamNodeByName(paraName);
                        //如果是对象类型
                        if (paramNode.getType().equalsIgnoreCase("_object")){
                            //根据对象名称获取对象
                            Type type = p.getType();
                    ...
                    if (stringResult != null) {
                        ...
                        handleResponseNode(responseNode, resultClassType.getElementType(),requestNode.getMethodName());
                    }
                    //设置响应信息
     ...
     * @param resultType
     */
    protected void handleResponseNode(ResponseNode responseNode, com.github.javaparser.ast.type.Type resultType,String methodName) {
        parseClassNodeByType(responseNode, resultType,methodName);
    }

    void parseClassNodeByType(ClassNode classNode, com.github.javaparser.ast.type.Type classType,String methodName) {
        // maybe void
        if (classType instanceof ClassOrInterfaceType) {
            ...
            ParseUtils.parseClassNodeByType(javaFile, classNode, classType,methodName);
        }
    }
