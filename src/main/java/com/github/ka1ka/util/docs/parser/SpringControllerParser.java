...
import com.github.ka1ka.util.docs.ParseUtils;
            ...
            if (paramNode != null) {
                ...
                p.getAnnotations().forEach(an -> {
                    ...
                    if ("RequestBody".equals(name)) {
                        setRequestBody(paramNode, p.getType(),requestNode.getMethodName());
                    }
                ...
                if (!paramNode.getJsonBody() && ParseUtils.isModelType(paramNode.getType())) {
                    ...
                    parseClassNodeByType(classNode, p.getType(),requestNode.getMethodName());
                    ...
                    /*requestNode.getParamNodes().remove(paramNode);
                    requestNode.getParamNodes().addAll(paramNodeList);*/
                    paramNode.setParamNodeList(paramNodeList);

                }

/*
                //如果参数是个对象 todo 新增对象判断
                ClassNode classNode = new ClassNode();
        ...
        //todo 新增 json格式
        /*requestNode.getParamNodes().forEach(paramNode -> {
            paramNode.get
        });*/
    ...
    @Override
    protected void handleResponseNode(ResponseNode responseNode, Type resultType,String methodName) {
        ...
        super.handleResponseNode(responseNode, resultType,methodName);
    }

    private void setRequestBody(ParamNode paramNode, Type paramType,String methodName) {
        if (ParseUtils.isModelType(paramType.asString())) {
            ClassNode classNode = new ClassNode();
            parseClassNodeByType(classNode, paramType,methodName);
